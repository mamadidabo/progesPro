...
var scheme = (function()
	...
	{
		//Attention, a mettre en commentaire après le developpement
		console.log(message);
	}
	...
	//Permet de vérifier si la fonction est dans l'environnement
	function find(o, x)
	{
	...
	function add_global(env){
		...
		//Rajout des fonctions standards non présente dans la classe Math
		env["#f"] = false;
		env["#t"] = true;
	...
	function evalte(x,env){
		env=env||global_env;
		//Vérifie si la sous-expression x est un String
		if (typeof x == "string")
			//retourne l'effet de la fonction x stocké dans l'environnement
			return find(env, x)[x];
		//Vérifie si la sous-expression x est un number
		else if (typeof x =='number')
			return x;
		//Vérifie si la sous-expression x est une citation
		else if (x[0] == 'quote')
			return x[1];
		//Vérifie si la sous-expression x est de type conditionnel
		else if (x[0] == 'if') {
			var test = x[1];
			var conseq = x[2];
			var alt = x[3];
		...
		//Vérifie si la sous-expresssion x est de type set
		else if (x[0] === 'set!'){
			find(env, x[1])[x[1]] = evalte(x[2], env);
			return "value of " + x[0] + " set"
		}
		//Vérirfie si sous-expression x est de type define
		else if (x[0] === 'define'){
			var variableDefined;
