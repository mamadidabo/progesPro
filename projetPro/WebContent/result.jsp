...
<% 
	...
	try 
	{
		...
		{
			%>Probleme avec la variable nbInput qui vaut  <%=request.getParameter("nbInput")%> <%
			serveurOk = false;
		}	
	} 
	...
	catch (Exception ex) 
	{
		...
		try 
		{
			...
			if(compteurErreur < 50) 
			{
				%><form id='form_erreur' action='result.jsp' method='GET'><%	
					for (int i=1;i<=nbInput;i++)
					{
						%><input type='hidden' id='textBox<%=i%>' name='textBox<%=i%>' value='<%=request.getParameter("textBox"+i)%>'/><%
					}
					%><input type='hidden' id='compteurErreur' name='compteurErreur' value='<%=compteurErreur+1%>'/><%
					%><input type='hidden' id='nbInput' name='nbInput' value='<%=request.getParameter("nbInput")%>'/><%
				%></form><% 
				%><script>document.getElementById("form_erreur").submit();</script><%
			}
		...
		catch (NumberFormatException e) 
		{
			%>
				Probleme 
				avec la variable compteurErreur  <%=request.getParameter("compteurErreur")%> 
				OU 
				avec la variable nbInput  <%=request.getParameter("nbInput")%> 
			<%
			serveurOk = false;
		}	
		...
		<div class='span6 offset3'>
			<table class='table table-bordered' id='tabResult'>
				<%
					...
					{
						%>		
							<caption>La formule est satisfiable, voici une solution :</caption>
							<tr>
								<%	
									...
									{	
										...
										%><td><img id='result<%=i%>' src='<%= FormulaBox.schemeExpressionToHTML(string) %>' border='0'/></td><%
										cpt++;
									}
					...
					{
						%><caption>La formule n'est pas satisfiable !</caption><%	
					}
				%>
			</table>
		</div>
...
<%}else{%>
	<div id='divResult' class='row-fluid' style='display:none'>
		<div class='span6 offset4' >
			Probleme de solveur, veuillez recharger la page
		</div>
	</div>
		<form id='form_rechargement' onsubmit="changePage('result.jsp');" method='GET'>
			<%	
				try 
				{
					int nbInput = Integer.parseInt(request.getParameter("nbInput"));
					...
					%><input type='hidden' id='compteurErreur' name='compteurErreur' value='1'/><%
					%><input type='hidden' id='nbInput' name='nbInput' value='<%=request.getParameter("nbInput")%>'/><%
				}
				catch (NumberFormatException e) 
				{
			...
			%>
				...
				<div class='row'>
					<div class='span6 offset5'>
						<button type='submit' class="btn btn-info btn-large"><i class="icon-white icon-refresh"></i></button>
					</div>
				</div>
		</form> 
<%}%>
